...
*/
        ...
        findViewById(R.id.allow_btn).setOnClickListener(new View.OnClickListener(){

                @Override
                public void onClick(View p1) {
                
                controlAction(intent, action, true);}});
        findViewById(R.id.deny_btn).setOnClickListener(new View.OnClickListener(){
    ...
    private void processRequest(Intent req_intent, @NonNull String action) {
        ...
        if(action.equals(ACTION_START)) {
            ...
            final CaptureSettings settings = new CaptureSettings(this, req_intent);
            ...
            if(!settings.pcap_uri.isEmpty()) {
                persistableUriPermission.checkPermission(settings.pcap_uri, settings.pcapng_format, new PersistableUriPermission.PupListener(){

                        @Override
                        public void onUriChecked(Uri granted_uri) {
            ...
            res.putExtra("version_name", "y");
            res.putExtra("version_code", "y");
            putStats(res, CaptureService.getStats());
        } else {
    ...
    private void getPeerInfo() {
        ...
        if((package_name == null) || !package_name.equals("" + ".debug")) {

    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        PasswordManager.requestPasswordAndSave(new Runnable() {
            ...
            public void run() {  
                ...
                setContentView(R.layout.fragpag);
    ...
    private void setupTabs() {
      //  mPagerAdapter = new StateAdapter(this);
       ...
       // ).attach();
        ...
        final Button buf=findViewById(R.id.activitymainButton1);
        buf.setText(R.string.status);
        buf.setOnClickListener(new OnClickListener(){

                @Override
                public void onClick(View p1) {
                    Fragment f=null;
                    String frag="";
                    if(buf.getText().toString().equals(getText(R.string.status))){
                        
                        f= EditListFragment.newInstance(ListInfo.Type.BLOCKLIST);
                        buf.setText(R.string.blocklist);
                    }else if(buf.getText().toString().equals(getText(R.string.blocklist))){
                        if(mHasWhitelist){
                            f= EditListFragment.newInstance(ListInfo.Type.FIREWALL_WHITELIST);
                            buf.setText(R.string.whitelist);
                        }else{
                            f=new FirewallStatus();
                            buf.setText(getText(R.string.status));
                        }
                    }else if(buf.getText().toString().equals(getText(R.string.whitelist))){
                        f=new FirewallStatus();
                        buf.setText(getText(R.string.status));
                    }
                    if(f!=null)

...
public class PayloadAdapter implements HTTPReassembly.ReassemblyListener {
    ...
    protected static class PayloadViewHolder {
        View headerLine;
        View dumpBox;
        TextView header;
        TextView dump;
...
/*
        ...
        final AdapterChunk chunk = getItem(payload_pos).adaptChunk;
        if (chunk == null)
            return;

        if(mMode == ChunkType.HTTP) {
            final String payload = chunk.getExpandedText(true);
            final int crlf_pos = payload.indexOf("\r\n\r\n");
            final String content_type = ((chunk.mChunk.contentType != null) && (!chunk.mChunk.contentType.isEmpty())) ?
            ...
            builder.setNeutralButton(R.string.cancel_action, new DialogInterface.OnClickListener(){

                    @Override
                    public void onClick(DialogInterface p1, int p2) {
                    }
                });
            builder.setPositiveButton(positive_action, new DialogInterface.OnClickListener() {
 ...
 /*                   else /* body */
    /*                    to_copy = to_copy.substring(crlf_pos + 4);
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
            builder.setTitle(title);
            builder.setSingleChoiceItems(choices.toArray(new String[]{}), mShowAsPrintable ? 0 : 1, new DialogInterface.OnClickListener(){

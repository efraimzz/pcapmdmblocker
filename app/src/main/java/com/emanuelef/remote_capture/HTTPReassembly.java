...
public class HTTPReassembly {
    ...
    public void handleChunk(PayloadChunk chunk) {
        ...
        if(mReadingHeaders) {
            ...
            try{
                BufferedReader reader = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(payload, 0, headers_size)));
        ...
        if(!mReadingHeaders) {
            ...
            if(mChunkedEncoding && (mContentLength < 0) && (body_size > 0)) {
                try{
                    BufferedReader reader = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(payload, body_start, body_size)));
                    String line = reader.readLine();
                    if(line != null) {
                        try {
    ...
    private void decodeBody(PayloadChunk body) {
        ...
        try{
            ByteArrayInputStream bis = new ByteArrayInputStream(body.payload);
